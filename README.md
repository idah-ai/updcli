...
```
src/
  datset.cr              Entry point / main
  command/
    base.cr              Abstract base class for all commands
    argument.cr          CLI argument record
    parser.cr            Argument parser
    error.cr             Error classes (Error, UpdError)
    init.cr              `init` command
    append.cr            `append` (bulk JSONL import) command
    merge.cr             `merge` command
    sign.cr              `sign` command
    verify.cr            `verify` command
    dataset/
      root.cr            `dataset` sub-command router
      create.cr
      list.cr
      show.cr
      update.cr
    ...
    entry/
      root.cr
      create.cr  list.cr  show.cr  update.cr  delete.cr
    annotation/
      root.cr
      create.cr  list.cr  show.cr  update.cr  delete.cr
    media/
      root.cr
      create.cr  list.cr  show.cr  update.cr  delete.cr  extract.cr
  upd/
    serialization.cr     RFC-compliant canonical serialisation and hashing
spec/
  command/
    dataset_spec.cr
    entry_spec.cr
...
sql/
  schema.sql             Core UPD schema (embedded at compile time)
Makefile
Dockerfile
shard.yml
```
